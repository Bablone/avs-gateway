...
AVS Gateway is an **Agent Admission Controller** for autonomous systems — like
a Kubernetes admission controller, but for AI agents. It intercepts proposed
actions before execution, evaluates policy, risk, trust, identity, and evidence,
then decides: allow, deny, require approval, or quarantine. Every decision
creates a verifiable ASR-1 receipt.

Payment rails like Mastercard AP4M and x402 handle money movement. AVS governs
whether the action itself — file access, API calls, deployments, database
queries, emails, or payments — should be allowed.
